(function (wp) {
  ...
  function modesFromSchema(schema) {
    if (Array.isArray(schema && schema.print_modes)) return schema.print_modes;
    var keys = (schema && schema.print_modes && schema.print_modes.keys) || ['SIMPLE_S1'];
    return keys.map(function (k) { return { key: k, label: k }; });
  }
  ...
  function preindex(rows) {
    ...
    Object.keys(idx).forEach(function (k) { idx[k].sort(function (a, b) { return a.q - b.q; }); });
    return idx;
  }
  ...
  function Configurator(props) {
    ...
    var modes = modesFromSchema(schema);
    ...
    var _useState3 = useState(modes[0] ? modes[0].key : 'SIMPLE_S1'), printModeKey = _useState3[0], setPrintModeKey = _useState3[1];
    ...
    var pricingIndex = useMemo(function () { return preindex(schema.pricing_rows || []); }, [schema]);
    ...
    function requestQuote() {
      ...
      wp.apiFetch({ path: restUrl + '/quote', method: 'POST', data: { product_id: productId, base_price_set_id: Number(schema.product_id || schema.id || productId), schema_version: Number(schema.schema_version || 1), selections: selections } }).then(function (response) {
        setQuote(response);
    ...
    return el('div', { className: 'swiftprint-configurator' },
      ...
      el('label', {}, 'Print mode'),
      el('select', { value: printModeKey, onChange: function (e) { setPrintModeKey(e.target.value); } }, modes.map(function (m) { return el('option', { key: m.key, value: m.key }, m.label || m.key); })),
      el('label', {}, 'Turnaround'),
